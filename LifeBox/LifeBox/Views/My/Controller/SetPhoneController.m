...
@implementation SetPhoneController

#pragma mark - 视图层
- (void)viewDidLoad {
    [super viewDidLoad];
    [self setWhiteNaviWithTitle:@"设定新手机"];
    self.view.backgroundColor = S_COBackground;
    keyStr = @"SetPhoneControllerKey";
    [self createUI];
}
...
- (void)createUI {
    ...
    newTF.font = [UIFont systemFontOfSize:Scale750(30)];
    newTF.keyboardType = UIKeyboardTypeNumberPad;
    newTF.borderStyle = UITextBorderStyleNone;
    newTF.clearButtonMode = UITextFieldViewModeWhileEditing;
    newTF.textColor = RGBColor(51, 51, 51);
    NSString *phoneHolder = @"请输入新手机号码";
    ...
    [placePhone addAttribute:NSFontAttributeName
                      ...
                      range:NSMakeRange(0, phoneHolder.length)];
    newTF.attributedPlaceholder = placePhone;
    [topView addSubview:newTF];
    [newTF mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerY.mas_equalTo(topView);
        make.left.mas_equalTo(Scale750(220));
        make.right.mas_equalTo(-Scale750(30));
        make.height.mas_greaterThanOrEqualTo(Scale750(90));
    }];
    /*
     * 分割线
     */
