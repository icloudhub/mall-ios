...
- (void)creatCellUI {
    ...
    oneData.imgStr = @"ic_collection";
    oneData.titleStr = @"收藏";
    PersonIconData *twoData = [[PersonIconData alloc] init];
    twoData.imgStr = @"ic_customer_service";
    twoData.titleStr = @"客服";
    PersonIconData *thrData = [[PersonIconData alloc] init];
    thrData.imgStr = @"ic_public_welfare";
    thrData.titleStr = @"公益";
    PersonIconData *fourData = [[PersonIconData alloc] init];
    fourData.imgStr = @"ic_invitation_gift";
    fourData.titleStr = @"邀请有礼";
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath {
    return CGSizeMake((Screen_width - Scale750(60))/4, Scale750(170));
}
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    if ([_tooltDelegate respondsToSelector:@selector(toolCellSelectWithTag:)]) {
        [_tooltDelegate toolCellSelectWithTag:indexPath.row];
    }
}
